...
两条边界刻意留在范围之外：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制；认证是一个独立、可选的能力 seam，本栅栏与它组合而非把它包进来（[可插拔的 Web 认证](2026-08-17-pluggable-web-authentication.md)）——这道栅栏本身是混淆代理人防御，而通过 `Host` 检查从不构成调用方身份的证据。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- **现在就上认证令牌。** 在本变更中否决：令牌的签发、存储、轮换是真实的产品面，而一套部署用来证明身份的机制与 rebinding 防御各自独立变化。栅栏在不预先决定那套设计的前提下就封死了浏览器混淆代理人漏洞；那套设计后来作为 `ctx.webAuth` seam 单独落地（[可插拔的 Web 认证](2026-08-17-pluggable-web-authentication.md)），叠加在本决策之上而非改动它。
