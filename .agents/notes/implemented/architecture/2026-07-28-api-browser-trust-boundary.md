...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and authentication is a separate, optional capability seam that this fence composes with rather than contains ([pluggable web authentication](2026-08-17-pluggable-web-authentication.md)) — the fence itself is a confused-deputy defense, and a passing `Host` is never evidence of who is calling. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- **Auth tokens as part of this fence.** Rejected: token minting, storage, and rotation are real product surface, and the mechanism a deployment proves identity with varies independently of the rebinding defense. The fence closes the browser-deputy holes without pre-deciding that design, which arrived separately as the `ctx.webAuth` seam ([pluggable web authentication](2026-08-17-pluggable-web-authentication.md)) and layers on this decision instead of altering it.
